...
import type { ContentsParams } from '../../src/api/contents';
...
export default function BlogScreen() {
  ...
  // Pre-select categories based on subscribed tags
  useEffect(() => {
  ...
  // Mark visited after 3s
  useEffect(() => {
    const timer = setTimeout(() => markVisited('blog'), 3000);
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['contents', { source_type: 'blog', q: submittedQuery, tags: tagsParam, page: 1, sort }],
    queryFn: () => getContents({ source_type: 'blog', q: submittedQuery || undefined, tags: tagsParam, page: 1, limit: 20, sort }),
  });

  useEffect(() => {
    setPage(1);
    setAllItems([]);
  }, [submittedQuery, tagsParam, sort]);
  ...
  const loadMore = useCallback(async () => {
    ...
    try {
      ...
      const result = await getContents({ source_type: 'blog', q: submittedQuery || undefined, tags: tagsParam, page: nextPage, limit: 20, sort });
  ...
  useEffect(() => {
    trackEvent([{ event_type: 'tab_visit', metadata: { tab: 'blog' } }]);
  }, []);

  // search tracking: when debounced query is submitted
  useEffect(() => {
    if (submittedQuery) {
      trackEvent([{ event_type: 'search', metadata: { tab: 'blog', q: submittedQuery } }]);
    }
  }, [submittedQuery]);
  ...
  useEffect(() => {
    ...
    trackEvent([{ event_type: 'filter_apply', metadata: { tab: 'blog', categories: selectedCategories, sort } }]);
  }, [selectedCategories, sort]);

  const renderItem = useCallback(
    ({ item }: { item: Content }) => (
      <ContentCard content={item} isNew={checkIsNew('blog', item.published_at)} />
    ),
    [checkIsNew]
  );
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => router.push('/bookmarks/blog')} style={styles.bookmarkBtn} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      <View style={styles.searchBar}>
        <TextInput style={styles.searchInput} value={query} onChangeText={handleChangeText} placeholder="블로그 검색..." placeholderTextColor={colors.textSecondary} returnKeyType="search" autoCorrect={false} autoCapitalize="none" />
      </View>
      ...
      {!isLoading && !isError && (
        <FlatList
          ...
          ListEmptyComponent={<View style={styles.center}><Text style={styles.emptyText}>블로그 포스트가 없습니다.</Text></View>}
          ListFooterComponent={
            hasMore ? (
